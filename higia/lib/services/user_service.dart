...
class UserService {
  ...
  static dynamic _serializeForSupabase(dynamic value) {
    ...
    if (value is Map)
      return value.map(
        (k, v) => MapEntry(k.toString(), _serializeForSupabase(v)),
      );
    if (value is List)
  ...
  Future<int?> fetchOrCreateAtividade(
    String? nivel,
  ...
  Future<int?> createUser(RegistrationData data) async {
    try {
      ...
      final idAtividade = await fetchOrCreateAtividade(
      ...
      final payload = serial is Map
          ? Map<String, dynamic>.from(serial)
          : <String, dynamic>{};
  ...
  Future<RegistrationData?> fetchUserData(int idutilizador) async {
    try {
      ...
      dynamic idAtvVal =
          row['idAtividade'] ?? row['idatividade'] ?? row['id_atividade'];
      int? idAtividade;
      if (idAtvVal is int)
        idAtividade = idAtvVal;
      else if (idAtvVal is String)
        idAtividade = int.tryParse(idAtvVal);
      ...
      if (idAtividade != null) {
        final ares = await _client
            ...
            .maybeSingle();
        if (ares != null) {
          final Map<String, dynamic> arow = Map<String, dynamic>.from(
            ares as Map,
          );
          final activityLevel =
              arow['AtividadeDiaria'] ??
              arow['atividade_diaria'] ??
              arow['nivel'] ??
              arow['atividade'];
          final atividadePreferida =
              arow['AtividadePreferida'] ?? arow['atividade_preferida'];
          if (activityLevel != null)
            reg.nivelAtividadeDiaria = activityLevel.toString();
          if (atividadePreferida != null)
            reg.AtividadePreferida = atividadePreferida.toString();
